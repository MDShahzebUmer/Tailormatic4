...
function getTabSect(str){
	...
	if(otitle=="etfabric"){
		...
		if(ttle=="19"){
			$("#menuopttitle-"+otitle).html("Choose Your Buttons :");
		} else if(ttle=="25"){
...
function validatebodyform2(){
	...
	if(document.getElementById('bsizeSleeve2').value==""){document.getElementById('bsizeSleeve2').focus(); return false;
		...
		rnge=$("#bsizeSleeve2").attr("data-title").split('-');frange=parseFloat(rnge[0]);trange=parseFloat(rnge[1]);
		if(typ=="cm"){frange=Math.round(frange*2.54,2);trange=Math.round(trange*2.54,2);} else {frange=frange;trange=trange;}
		if(IsFloat(document.getElementById('bsizeSleeve2').value)==false){
			$("#bsizeSleeve2").css({'border-color':'#f00','box-shadow':'0px 0px 15px #f00','-webkit-box-shadow':'0px 0px 15px #f00'});
