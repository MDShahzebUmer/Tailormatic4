...
function frontProcessing(fArray){
	...
	$.each(frontArr,function(key,value){
		if(value!=""){
			cdata = "";
			setInterval(fabric.Image.fromURL(frontArr[key], function(oImg) {
				canvas.add(oImg);
...
function backProcessing(bArray){
	...
	$.each(BackArr,function(key,value){
		if(value!=""){
			cdata = "";
			setInterval(fabric.Image.fromURL(BackArr[key], function(oImg) {
				canvas.add(oImg);
				cdata=canvas.toDataURL();
...
function getfab(id,jArray,otab){	
    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
       ...
       success:function(data){		   
		   	setTimeout($('body').html(data),2000);
       }
    });
}
function getstyles(id,ctyp,jArray,otab){
    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
       ...
       success:function(data){
			setTimeout($('body').html(data),1000);
	   }
    });
}
...
function addMoreSize(){
	...
	$("#stdullst").append('<ul class="add_other_size cl'+cl+'"><li><span class="longarrow"><i class="fa fa-long-arrow-right" aria-hidden="true"></i></span><span></span></li><li><div class="et-btn-group"><div class="et-btn-select">'+ sizedv +'</div></div></li><li><span>Quantity :</span></li><li><div class="et-btn-group"><div class="et-btn-select">'+ qtydv +'</div></div></li><li><a href="javascrit:void(0);" onClick="javascript:delQty(\'cl'+cl+'\');"><i class="fa fa-minus-circle" aria-hidden="true"></i></a></li></ul>');
}
