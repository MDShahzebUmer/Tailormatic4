...
function viewMainBackJacket(){
	$('#prev_img_jacket #main-front').css('display','none');
	$('#prev_img_jacket #main-side').css('display','none');
	$('#prev_img_jacket #main-back').css('display','block');
}
function viewMainFrontJacket(){
	$('#prev_img_jacket #main-front').css('display','block');
	$('#prev_img_jacket #main-side').css('display','none');
	$('#prev_img_jacket #main-back').css('display','none');
}
function viewMainSideJacket(){ 
	$('#prev_img_jacket #main-front').css('display','none');
	$('#prev_img_jacket #main-side').css('display','block');
	$('#prev_img_jacket #main-back').css('display','none');
}
...
function sideJacketProcessing(){
	...
	fabric.Image.fromURL(_src, function(oImg) {
		...
		if (retjacketsidesrc.length > 0) { setTimeout(sideJacketProcessing, 40); } else{ $("#prev_img_jacket div[id^='main-side']").find("div.pt-jacketimage-div img").attr("src",cdata);
...
function backJacketProcessing(){
	...
	fabric.Image.fromURL(_src, function(oImg) {
		...
		if (retjacketbacksrc.length > 0) { setTimeout(backJacketProcessing, 40); 
			...
			$("#prev_img_jacket div[id^='main-back']").find("div.pt-jacketimage-div img").attr("src",cdata);
...
function frontJacketProcessing(){
	...
	fabric.Image.fromURL(_src, function(oImg) {
		...
		if (retjacketfrontsrc.length > 0) {setTimeout(frontJacketProcessing, 40);} else {
		$("#prev_img_jacket div[id^='main-front']").find("div.pt-jacketimage-div img").attr("src",cdata);
		$("#miniview-etstylejacket-19").find("figure img").attr("src",cdata);$("#miniview-etstylejacket-20").css("background-image","url("+cdata+")");$("#miniview-etstylejacket-21").css("background-image","url("+cdata+")");$("#miniview-etstylejacket-22").css("background-image","url("+cdata+")");$("#miniview-etcontrastjacket-25").find("figure img").attr("src",cdata);
		}
	});
}
...
function getjacketfab(id,otab){	
    ...
    $.ajax({
       	type:'POST',
       	// url:'/getfabricsjacket2',
		url:'/getfabricsjacket22',
       	...
       	success:function(data){
			...
			$("#container_jackets .vwprice").html("2Piece Suit ( 1 Jacket, 1 Pant ) : $ "+total_price);

			// $("#twopiece_total").html("$ "+total_price);
			$(".t-s-price").html("$"+total_price);
		}
    });	
}
...
function showJacketMeasureSect(id){
	...
	if(id=="bodysize" || id=="standardsize" || id=="outfitsize"){
		...
		if(id=="bodysize"){ 
			...
			var fview=$("#prev_img_jacket #main-front").find("div.pt-jacketimage-div img").attr("src"); 
			var bview=$("#prev_img_jacket #main-back").find("div.pt-jacketimage-div img").attr("src");
	...
	if(main_nav_str=="etfabric"){
		...
		switch(stab){
			case "19":
				...
				break;
			case "20":
				openPgContent('menu-19','etstylejacket','19','','style');
				break;
			case "21":
				openPgContent('menu-20','etstylejacket','20','','style');
				break;
			case "22":
				openPgContent('menu-21','etstylejacket','21','','style');
				break;
			case "23":
				openPgContent('menu-22','etstylejacket','22','','style');
				break;
			case "24":
				openPgContent('menu-23','etstylejacket','23','','style');
				break;
		}
		...
		switch(stab){
			...
			case "26":
				...
				// openPgContent('menu-40','etcontrast','40','','contrast');
			...
			case "27":
				...
				break;
			case "28":
				openPgContent('menu-27','etcontrastjacket','27','','contrast');
				break;
			case "29":
				openPgContent('menu-28','etcontrastjacket','28','','contrast');
				break;
		}
	}
}
	...
	if(main_nav_str=="etfabric"){
		...
		switch(stab){
			...
			case "22":
				...
				break;
			case "23":
				openPgContent('menu-24','etstylejacket','24','','style');
				break;
			case "24":
				showStyle('pants');
				openPgContent('menu-48','etstylepant','48','','style');
				break;
		}
	} else if(main_nav_str=="etcontrast"){
		var stab = "";
		...
		switch(stab){
			...
			case "27":
				...
				break;
			case "28":
				openPgContent('menu-29','etcontrastjacket','29','','contrast');
				break;
			case "29":
				openNav('etmeasurement');
				break;
		}
	}	
}
