...
$style = [
    ...
    'td-contents-strong' => 'width: 90px; float: left; font-weight: 700;',
    'p-margin0' => 'margin: 0;',
    'p-marginBottom' => 'margin: 0; margin-bottom: 10px;',
    'footer-td-wrap' => 'padding: 8px; line-height: 1.42857143; vertical-align: top; border-top: 1px solid #ddd; font-style: italic;background: #593A2E; font-size: 15px; letter-spacing: 1px; color: #fff; font-family: "Arial";',
...
<table cellpadding="0" cellspacing="0" style="{{ $style['table-body'] }}">
	    <thead>
	      ...
	      <tr>
	        <td colspan="6" style="{{ $style['invoice-header'] }}"><a href="{{url('/')}}"><img style="{{ $style['logo-img'] }}" src="{{asset('storage/')}}/{!! setting('site.logo') !!}"></a></td>
	      </tr>
	    </thead>
	    <tbody>
	        <tr>
	        ...
	        <tr>
                ...
                <img src="{{URL::asset('/storage/'.$cart->fabric_image)}}" width="50"  alt="">
                @endif
                <br><span>{{$groupname}}</span></td>
            ...
            <tr>
	            <td style="{{ $style['td-contents'] }}" align="right" colspan="5">
                  ...
                  <?php if($camt!=0){?>
                   <p style="{{ $style['p-marginBottom'] }}"><strong>Coupon({{$ccode}}): </strong></p>
                   <?php }?>
                    <?php if($taxamt!=0){?>
  ...
  <button onclick="myFunction()" class="et-btn et-btn2">Print</button>
  <script>
function myFunction() {
    window.print();
}
